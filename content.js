...
async function getENSRecords(domain) {
  // Call the ENS API or web3.eth.ens API to retrieve the records for the domain.
  // Return an object with the records.
  
  const provider = new InfuraProvider('mainnet', 'ec7a6f95b3cd43b1b3773039ab33de0a'); // connect to Ethereum network
...
function showPopUp(text, records) {
  // Create a pop-up element and set its content to the records.
  // Position the pop-up below the text element.
  const popUp = document.createElement("div");
  popUp.setAttribute("id", "ens-popup");
  popUp.style.position = "absolute";
  popUp.style.top = (text.getBoundingClientRect().bottom + window.pageYOffset) + "px";
  popUp.style.left = text.getBoundingClientRect().left + "px";
  ...
  for (const [key, value] of Object.entries(records)) {
    ...
    recordItem.innerHTML = `<b>${key}</b>: ${value}`;
    recordList.appendChild(recordItem);
  }
  popUp.appendChild(recordList);
  document.body.appendChild(popUp);
}

function hidePopUp() {
  // Remove the pop-up element.
  const popUp = document.getElementById("ens-popup");
  if (popUp) {
    popUp.remove();
  }
}
...
async function handleMouseOver(event) {
  ...
  if (text.textContent.endsWith(".eth")) {
    const domain = text.textContent.trim();
    const records = await getENSRecords(domain);
    if (Object.keys(records).length > 0) {
      showPopUp(text, records);
    }
  }
}
